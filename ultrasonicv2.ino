 ...
 * 版本：只用两个 HC-SR04；假设小车启动时基本朝向墙体。
 * 目标：对齐墙面(|dL-dR|<=ALIGN_EPS_CM) -> 慢速到 8±1 cm -> 停车；
 *      任一侧<MIN_SAFE_CM 立即急停；两侧都无效时小角速度原地旋转兜底。
 * 说明：沿用 reference 的电机引脚/宏风格；analogWrite 全部使用 0~255。
 */
 ...
 #define MIN_SAFE_CM      5
 #define MAX_VALID_CM     350   // 放宽上限，避免空旷处被误判无效
 ...
 long smoothUltraCM(uint8_t trig, uint8_t echo){
   ...
   for(int i=0;i<N;i++){
     v[i]=readUltrasonicCM(trig,echo);
     if(v[i]<0) return -1;
   }
 ...
 // ⚠ 若你发现“左转/右转”方向与实际相反，只需对调下面两行里调用的 rotate_1 / rotate_2
 inline void turnLeft(){  Motor_PWM = SPD_TURN; rotate_2(); }
 ...
 void loop(){
   ...
   if (millis() - lastTick >= 15){
     ...
     // 兜底：如果两边都无效，强制原地慢转，直到转到能双侧有效（避免“看起来不动”）
     bool bothInvalid = (!Lok && !Rok);
     if (state != FAIL && bothInvalid){
       Motor_PWM = SPD_TURN;
       rotate_2(); // 若方向不对，改成 rotate_1()
       return;     // 本周期只做兜底旋转
     }
     ...
     switch(state){
       ...
       case ALIGN_WALL: {
         ...
         if (fabs(diff) <= ALIGN_EPS_CM){
           ...
           if (display.width()>0){
             ...
             display.setTextSize(2); display.println("Aligned"); display.display();
           }
         }else{
       ...
       case PARK_8CM: {
         ...
         if (avg > TARGET_DIST_CM + TARGET_EPS_CM){
           ...
           driveBackwardSlow(); // 轻微倒回，避免贴墙
         }else{
           driveStop(); state = DONE;
           if (display.width()>0){
             display.clearDisplay(); display.setCursor(0,0);
             display.setTextSize(2); display.println("PARKED 8cm"); display.display();
           }
         }
       } break;
       ...
       case FAIL:
         ...
         if (display.width()>0){
           display.clearDisplay(); display.setCursor(0,0);
           display.setTextSize(2); display.println("EMERGENCY!"); display.display();
         }
         break;
     }
   }
